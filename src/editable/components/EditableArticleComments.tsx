...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  const persist = (next: Comment[]) => {
    ...
    try {
      ...
      // Ignore storage failures and keep in-memory comments.
    }
  }
  ...
  return (
    <section className="mt-16 border-t border-[var(--tk-line)] pt-12">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="editable-display text-4xl font-semibold tracking-[-0.04em] text-[var(--tk-text)]">Comments</h2>
          <p className="text-sm text-[var(--tk-muted)]">{all.length} responses so far</p>
        </div>
      </div>

      <form onSubmit={submit} className="mt-8 rounded-[1.9rem] border border-[var(--tk-line)] bg-white p-6 shadow-[0_20px_60px_rgba(18,45,108,0.08)]">
        <div className="grid gap-3 md:grid-cols-[220px_minmax(0,1fr)]">
          <input
            ...
            maxLength={60}
            className="h-12 w-full rounded-full border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
          />
          <textarea
            value={text}
            ...
            rows={3}
            maxLength={1500}
            className="w-full resize-y rounded-[1.4rem] border border-[var(--tk-line)] bg-[var(--tk-bg)] px-4 py-3 text-sm leading-7 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
          />
        </div>
        <div className="mt-4 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent)] px-6 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[var(--tk-on-accent)] transition hover:brightness-95 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid gap-4">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[1.7rem] border border-[var(--tk-line)] bg-white p-5 shadow-[0_16px_42px_rgba(18,45,108,0.06)]">
            <div className="flex items-center gap-3">
