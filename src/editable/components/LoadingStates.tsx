...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[1.5rem] bg-[var(--slot4-accent-soft)]', className)} />
}
...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--slot4-muted-text)]">{label}</p>
      <PulseBlock className="mt-5 h-14 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[2rem] border border-[var(--editable-border)] bg-white p-5 shadow-[0_18px_50px_rgba(18,45,108,0.08)]">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[1.85rem] border border-[var(--editable-border)] bg-white p-4 shadow-[0_18px_50px_rgba(18,45,108,0.08)]">
